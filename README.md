...
- **教程与示例**在空工作区欢迎页和文档栏中提供三份完整、虚构且游戏中立的 `.kirin`：基础公式、参数方案与比较、扫描与图表。教程源码保持只读，不参加当前工作区的校验、计算、搜索或保存；只有用户主动“复制为未保存草稿”后，展开后的源码才进入工作区流程。
...
首次打开空工作区时，浏览器工作台会显示只读教程目录。这些安装资源不是工作区文档，也不会被隐式保存；用户可以查看完整源码，再用自选文档 ID 复制为普通未保存草稿。
