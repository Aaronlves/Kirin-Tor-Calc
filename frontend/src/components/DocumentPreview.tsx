...
import { FileOutput, Play, Save } from "lucide-react";
    ...
    <>
      <ScrollArea h="100%" type="auto">
        <Stack p="md" gap="md">
          ...
          {entryTargets.length > 0 && hasChart && <SegmentedControl fullWidth size="xs" value={mode} onChange={(value) => { setMode(value as "result" | "chart"); setResult(null); }} data={[{ value: "result", label: "结果" }, { value: "chart", label: "图表" }]} />}
          ...
          <Select label="参数方案" clearable searchable placeholder="源码默认值" value={preset} onChange={setPreset} data={presetOptions} />
          <Textarea label="临时参数" description="只覆盖本次投影，不构成文档权威。" placeholder="暴击率=25%，天赋=true" autosize minRows={3} maxRows={7} value={overrides} onChange={(event) => setOverrides(event.currentTarget.value)} />
          ...
          {result && mode === "chart" && <Stack gap="sm" className="document-chart-preview"><ChartCanvas result={result} /><Group gap={6}><Badge variant="light" color="green">{Array.isArray(result.rows) ? result.rows.length : 0} 个采样点</Badge><Badge variant="outline" color="gray">{String(result.x || "—")}</Badge></Group><TechnicalResult result={result} /></Stack>}
