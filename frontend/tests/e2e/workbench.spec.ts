...
function waitForCompletionResponse(page: Page, prefix: string, explicit: boolean) {
  return page.waitForResponse((response) => {
    ...
    return payload?.prefix === prefix && payload.explicit === explicit;
  });
}
...
test.describe.serial("Kirin Tor 浏览器工作台交互", () => {
  ...
  test("topic 发现只展示兼容 manifest 与仓库链接", async ({ page }) => {
    ...
    let subview = pluginManager.getByRole("region", { name: "发现社区 Workbench Plugins" });
    await expect(subview.getByText("Community Browser", { exact: true })).toBeVisible();
    await expect(subview.getByText("kirin-tor-plugin", { exact: true })).toBeVisible();
    await expect(subview.getByText("未审核", { exact: true })).toBeVisible();
    await expect(subview.getByRole("link", { name: "在 GitHub 查看" })).toHaveAttribute("href", "https://github.com/community/plugin-example");
    await expect(subview.getByRole("button", { name: /安装/ })).toHaveCount(0);
    await expect(page.locator('[aria-modal="true"]:visible')).toHaveCount(1);
    await subview.getByRole("button", { name: "返回", exact: true }).click();
    await expect(subview).toBeHidden();
    ...
    const packageManager = page.getByRole("dialog", { name: "Package 管理" });
    subview = packageManager.getByRole("region", { name: "发现社区 Packages" });
    await expect(subview.getByText("community.example", { exact: true })).toBeVisible();
    await expect(subview.getByText("kirin-tor-package", { exact: true })).toBeVisible();
    await expect(subview.getByText("namespace community_example", { exact: true })).toBeVisible();
    await expect(subview.getByRole("link", { name: "在 GitHub 查看" })).toHaveAttribute("href", "https://github.com/community/package-example");
    await expect(page.locator('[aria-modal="true"]:visible')).toHaveCount(1);
  });
  ...
  test("主工作台与语法抽屉通过自动无障碍检查", async ({ page }) => {
    ...
    await createDialog.getByRole("button", { name: "关闭对话框" }).click();
    await page.getByRole("button", { name: "设置", exact: true }).click();
  ...
  test("页面、工具与子视图保持连续标题和单一模态层", async ({ page }) => {
    await openWorkbench(page);
    const main = page.getByRole("main");
    await expect(page.getByRole("heading", { level: 1, name: "文档" })).toHaveCount(1);
    await expect(main.getByRole("region", { name: "文档索引" })).toBeVisible();
    await expect(main.getByRole("region", { name: /源码编辑器/ })).toBeVisible();
    await expect(main.getByRole("complementary", { name: "文档检查器" })).toBeVisible();

    await page.getByRole("button", { name: "关系图", exact: true }).click();
    await expect(page.getByRole("heading", { level: 1, name: "关系图" })).toHaveCount(1);
    await expect(main.getByRole("heading", { level: 2, name: "关系图" })).toBeVisible();
    await expect(main.getByRole("region", { name: "关系图画布" })).toBeVisible();
    await expect(main.getByRole("region", { name: "所选关系节点" })).toBeVisible();

    await page.getByRole("button", { name: "工作区工具", exact: true }).click();
    await page.getByRole("menuitem", { name: "运行记录", exact: true }).click();
    let tool = page.getByRole("dialog", { name: "运行记录" });
    await expect(tool).toBeVisible();
    await expect(tool.getByRole("heading", { level: 3, name: "记录与重放" })).toBeVisible();
    await expect(tool.locator("h4").first()).toBeVisible();
    expect(await tool.locator("h1,h2,h3,h4,h5,h6").evaluateAll((headings) => headings.map((heading) => Number(heading.tagName.slice(1))))).toEqual([2, 3, 4]);
    await expect(page.locator('[aria-modal="true"]:visible')).toHaveCount(1);
    await tool.getByRole("button", { name: "关闭工作区工具" }).click();

    await page.getByRole("button", { name: "工作区工具", exact: true }).click();
    await page.getByRole("menuitem", { name: "Package 管理", exact: true }).click();
    tool = page.getByRole("dialog", { name: "Package 管理" });
    await tool.getByRole("button", { name: "发现 Package" }).click();
    const discovery = tool.getByRole("region", { name: "发现社区 Packages" });
    await expect(discovery).toBeVisible();
    await expect(page.locator('[aria-modal="true"]:visible')).toHaveCount(1);
    expect(await tool.locator("h1,h2,h3,h4,h5,h6").evaluateAll((headings) => headings.map((heading) => Number(heading.tagName.slice(1))))).toEqual([2, 3, 4]);
    await page.keyboard.press("Escape");
    await expect(discovery).toBeHidden();
    await expect(tool.getByRole("heading", { level: 3, name: "可复现的社区预设" })).toBeVisible();
    await expect(page.locator('[aria-modal="true"]:visible')).toHaveCount(1);
  });

  test("浮动表面使用 design layer token 的运行时层级", async ({ page }) => {
    const layer = (JSON.parse(await readFile(resolve("src/design/tokens.json"), "utf8")) as { layer: Record<string, string> }).layer;
    await openWorkbench(page);

    await page.getByRole("button", { name: "工作区工具", exact: true }).click();
    await expect(page.locator(".mantine-Menu-dropdown")).toHaveCSS("z-index", layer.popover);
    await page.keyboard.press("Escape");
    ...
    await page.getByRole("button", { name: /命令/ }).click();
    await expect(page.locator(".mantine-Spotlight-inner")).toHaveCSS("z-index", layer.spotlight);
    await page.getByPlaceholder("搜索页面或命令…").fill("检查工作区");
    await page.getByText("检查工作区", { exact: true }).click();
    await expect(page.locator('.mantine-Notifications-root[data-position="top-right"]')).toHaveCSS("z-index", layer.notification);
  });

  test("框架几何变量、管理抽屉与检查器保持受控宽度", async ({ page }) => {
    await openWorkbench(page);
    expect(await page.locator("html").evaluate((element) => getComputedStyle(element).getPropertyValue("--mantine-scale").trim())).toBe("1");

    await page.getByRole("button", { name: "设置", exact: true }).click();
    const settings = page.getByRole("dialog", { name: "工作台设置" });
    await expect(settings).toBeVisible();
    expect(Math.round((await settings.boundingBox())?.width ?? 0)).toBe(760);
    await settings.getByRole("button", { name: "关闭工作区工具" }).click();

    await page.getByRole("button", { name: "工作区工具", exact: true }).click();
    await page.getByRole("menuitem", { name: "Package 管理", exact: true }).click();
    const packages = page.getByRole("dialog", { name: "Package 管理" });
    await expect(packages).toBeVisible();
    expect(Math.round((await packages.boundingBox())?.width ?? 0)).toBe(960);
    await packages.getByRole("button", { name: "关闭工作区工具" }).click();

    await openCombo(page);
    const trialRow = page.locator(".trial-input-row").first();
    await expect(trialRow).toBeVisible();
    expect(await trialRow.evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBeTruthy();
  });
  ...
  test("宿主与编辑器 Tooltip 使用统一表面并留在各自边界内", async ({ page }) => {
    ...
    await expect(hostTooltip).toHaveCSS("background-color", "rgb(32, 33, 29)");
    ...
    await expect(editorTooltip).toHaveCSS("background-color", "rgb(32, 33, 29)");
  ...
  test("文档行状态覆盖操作按钮且折叠导航保持居中", async ({ page }) => {
    ...
    await expect(inactiveRow).toHaveCSS("background-color", "rgb(37, 40, 33)");
  ...
  test("通知自动收起，Windows 快捷键与真实文件改名入口可用", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "保存全部" })).toHaveCount(1);
    await expect(page.getByRole("button", { name: "命令 Ctrl+K" })).toBeVisible();
    ...
    await pathInput.fill("entries/archive/renamed-combo.kirin");
    await moveDialog.getByRole("button", { name: "应用文件路径" }).click();
    await expect(page.getByRole("button", { name: "entries/archive 1", exact: true })).toBeVisible();
    await expect(access(resolve(".e2e-workspace", "entries", "archive", "renamed-combo.kirin"))).resolves.toBeUndefined();

    const renamedButton = page.getByRole("button", { name: /^双技能组合（虚构） entries\/archive\/renamed-combo\.kirin/ });
    await page.getByRole("button", { name: "文档操作：双技能组合（虚构）" }).click();
    await page.getByRole("menuitem", { name: "重命名 / 移动真实文件" }).click();
    ...
    await page.getByRole("button", { name: "设置", exact: true }).click();
    const settingsDrawer = page.getByRole("dialog", { name: "工作台设置" });
    await expect(settingsDrawer.getByText("当前工作区", { exact: true })).toBeVisible();
    await expect(settingsDrawer.getByText("反馈与通知", { exact: true })).toBeVisible();
    await expect(settingsDrawer.getByText("快捷键", { exact: true })).toBeVisible();
    await expect(settingsDrawer.getByText("Ctrl+S", { exact: true })).toBeVisible();
    await expect(settingsDrawer.getByRole("button", { name: "Package 管理" })).toBeVisible();
    await expect(settingsDrawer.getByRole("button", { name: "Workbench Plugins" })).toBeVisible();
    await settingsDrawer.getByRole("combobox", { name: "普通通知停留时间" }).click();
    ...
    try {
      ...
      await editor.type("\n// windows shortcut save");
      await page.keyboard.press("Control+s");
      await expect(page.getByText("已保存", { exact: true })).toBeVisible();
      await expect.poll(async () => readFile(sourcePath, "utf8")).toContain("// windows shortcut save");
      await expect(page.getByText("已保存", { exact: true })).toBeHidden({ timeout: 8000 });
    } finally {
  ...
  test("工作区搜索生成可审查草稿，相关语法可从诊断直达", async ({ page }) => {
    ...
    const firstSearchResult = searchDrawer.getByRole("list", { name: "工作区搜索结果" }).getByRole("listitem").first();
    await expect(firstSearchResult).toBeVisible();
    await expect(firstSearchResult.getByRole("button", { name: /^打开搜索结果/ })).toBeVisible();
  ...
  test("可以放弃全部已有草稿，也可以移除未落盘的新文档草稿", async ({ page, request }) => {
    await openWorkbench(page);
    await openCombo(page);
    ...
    editor = page.getByRole("textbox", { name: "Kirin Tor 源码：技能 A（虚构）" });
    await editor.press(`${modKey}+End`);
    await editor.type("\n// discard skill draft");
    await expect(page.locator('[aria-label="工作区状态：2 个草稿"]')).toBeVisible();

    await page.getByRole("button", { name: "工作区工具" }).click();
    await page.getByRole("menuitem", { name: "保存前变更审查" }).click();
    const review = page.getByRole("dialog", { name: "保存前变更审查" });
    await review.getByRole("button", { name: "放弃全部草稿" }).click();
    const discardAll = review.getByRole("region", { name: "放弃全部未保存草稿" });
    await expect(discardAll).toContainText("2 个未保存草稿");
    await discardAll.getByRole("button", { name: "确认放弃" }).click();
    await expect(review.getByText("当前没有未保存草稿。", { exact: true })).toBeVisible();
    let bootstrap = await request.get("/api/bootstrap", { headers: { "X-Kirin-Token": "kirin-e2e-token" } });
    expect((await bootstrap.json()).recovery.drafts).toEqual({});
    await review.getByRole("button", { name: "关闭工作区工具" }).click();
    await expect(page.locator('[aria-label="工作区状态：工作区有效"]')).toBeVisible();

    await page.getByRole("button", { name: "新建文档" }).click();
    const createDialog = page.getByRole("dialog", { name: "新建文档" });
    await createDialog.getByRole("textbox", { name: "文档 ID" }).fill("discard_me");
    await createDialog.getByRole("button", { name: "创建草稿" }).click();
    await expect(page.getByRole("textbox", { name: "Kirin Tor 源码：discard_me" })).toBeVisible();
    await page.getByRole("button", { name: "文档操作", exact: true }).click();
    await page.getByRole("menuitem", { name: "放弃当前草稿" }).click();
  ...
  test("补全片段替换前缀并把光标放进参数", async ({ page }) => {
    ...
    await editor.press("Enter");
    const excludedCompletionResponse = waitForCompletionResponse(page, "条件筛选最小值", false);
    await page.keyboard.insertText("条件筛选最小值");
    const excludedCompletionPayload = await (await excludedCompletionResponse).json() as {
      items: Array<{ label: string }>;
    };
    expect(excludedCompletionPayload.items.map((item) => item.label)).not.toContain("条件筛选最小值");
  ...
  test("上下文补全、自动缩进、行注释与精确参考入口共享 authoring contract", async ({ page }) => {
    ...
    await editor.press("Enter");
    const stateCompletionResponse = waitForCompletionResponse(page, "sta", false);
  ...
  test("编辑器提供查找、大纲、源码联动、引用与安全重命名", async ({ page }) => {
    ...
    await expect(searchInput).toHaveCSS("background-color", "rgb(18, 19, 16)");
  ...
  test("检查器提供非权威试算、运行记录、preset 草稿与多图排列", async ({ page, request, browserName }) => {
    ...
    let compactChart = page.locator(".preview-chart-card").filter({ hasText: "短区间组合曲线" });
    await compactChart.locator(".canvas-data-fallback summary").click();
    const targetSource = compactChart.getByRole("button", { name: "定位 组合期望伤害 源码" });
    await targetSource.focus();
    await targetSource.press("Enter");
    await expect(page.getByRole("radio", { name: "分栏" })).toBeChecked();
    await expect(page.locator(".cm-activeLine")).toContainText('total "组合期望伤害"');
    await page.locator(".mantine-SegmentedControl-label").filter({ hasText: "图表" }).click();
    await page.locator(".document-focus-switch .mantine-SegmentedControl-label").filter({ hasText: "仅预览" }).click();
    compactChart = page.locator(".preview-chart-card").filter({ hasText: "短区间组合曲线" });
    const compactCanvas = compactChart.locator(".chart-canvas");
    const compactBox = await compactCanvas.boundingBox();
    ...
    await expect(tooltip).toHaveCSS("background-color", "rgb(32, 33, 29)");
  ...
  test("固定策略由 Process 重放并证明边界周期", async ({ page }) => {
    ...
    const trajectoryCanvas = preview.locator(".preview-chart-card").first().locator(".chart-canvas");
  ...
  test("诊断项跳转到编辑器中的错误位置", async ({ page }) => {
    ...
    await expect(page.getByRole("radio", { name: "当前文档 0" })).toBeChecked();
  ...
  test("切换工作区时保留当前草稿并隔离新工作区状态", async ({ page, request }) => {
    ...
    const recoveredEditor = page.getByRole("textbox", { name: "Kirin Tor 源码：双技能组合（虚构）" });
    await recoveredEditor.press(`${modKey}+f`);
    await page.locator('.cm-search input[name="search"]').fill("workspace switch recovery");
    await page.locator('.cm-search button[name="next"]').click();
    await page.keyboard.press("Escape");
    await expect(page.locator(".cm-activeLine")).toContainText("// workspace switch recovery");

    await page.getByRole("button", { name: "工作区工具" }).click();
    await page.getByRole("menuitem", { name: "保存前变更审查" }).click();
    const review = page.getByRole("dialog", { name: "保存前变更审查" });
    await review.getByRole("button", { name: "放弃全部草稿" }).click();
    await review.getByRole("region", { name: "放弃全部未保存草稿" }).getByRole("button", { name: "确认放弃" }).click();
    await expect(page.locator('[aria-label="工作区状态：工作区有效"]')).toBeVisible();
  });
});
