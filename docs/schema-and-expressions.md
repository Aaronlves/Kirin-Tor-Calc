...
该容错索引同时提供源码大纲、定义位置、直接引用、别名引用、悬浮签名和函数参数信息。`F2` 重命名只适用于可写正式成员：它保留中文标签与别名，统一改写定义、同文档短引用和跨文档正式引用，并在返回内存草稿前执行完整工作区校验。索引、引用列表、恢复缓存和改名预览都不是独立权威；只有最终保存的 `.kirin` 文本参与持久事实。
