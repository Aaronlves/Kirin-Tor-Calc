...
作者、Agent 或普通编辑器直接写入本地 `.kirin` 都不会形成新的来源层级。磁盘内容可以暂时无效，但
必须通过同一解析、引用、类型、单位、边界和 Process 检查后才能求值。浏览器中的未保存 overlay 不会
自动暴露给外部 Agent；外部磁盘修改与 dirty overlay 相遇时必须保留二者并进入显式冲突处理。

每个文件只声明一个 `entry`。正式 ID、成员、类型、对象和属性名使用
`[A-Za-z_][A-Za-z0-9_]*`，且不能以 `__` 开头，也不能使用 Python 关键字或表达式字面量
（`true`、`false`、`True`、`False`、`None`、`empty`）。文件路径、注释和显示标签都不构成引用身份。

解析器拒绝 v1、未知声明或 block 属性、重复成员或 block 属性、未知对象属性、缺少的必填属性、
Tab 缩进和不一致的嵌套。不定义多行表达式续行的叶声明不接受缩进子块。说明围栏中的内容是例外：
它作为不透明 UTF-8 文本保存。
...
静态 scalar 输入、字段、函数和输出只接受数值或布尔 domain；symbolic domain 当前用于封闭结构字段
与类型化 Process。字段、函数和输出的声明类型属于校验契约，不能用同量纲但不同 value type 的表达式替代。
布尔字面量只有小写 `true`、`false` 两种正式写法。
...
默认值的首次发生、条件筛选最小值、条件首次成立前的最后值、停止时间、最大回撤、总变化量和时间
加权方差；条件筛选为空或不存在前置样本时使用作者声明的显式默认值。派生 Measure 可以引用其他
Measure 组成普通安全表达式。具名 Objective 仅引用数值 Measure，支持任意有限层字典序目标和布尔约束。有限
