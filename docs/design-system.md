...
Motion is productive rather than decorative. Ordinary interactive surfaces use the single neutral `surface.hover` color for pointer hover and the fast transition duration; selected, primary, and destructive states retain their separate semantic colors. Disclosure and document-group chevrons use the standard duration to preserve orientation while expanding. Drawer, Modal, Menu, Popover, and Tooltip transitions use the same motion tokens. Data values must not tween in a way that implies recalculation. Under `prefers-reduced-motion: reduce`, host transitions become instant. Pointer-only presentation must retain a keyboard-readable or keyboard-operable equivalent.
