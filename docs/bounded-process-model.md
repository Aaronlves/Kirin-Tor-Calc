...
- 指定一个固定 action 序列；
- 提供一个由纯表达式构成的确定策略；
- 在公开事件结算后或状态条件从假到真时产生决策点；
- 用固定周期产生作者主动选择的离散决策点；
- 声明有限个有界连续动作时点，使用次数可从零到声明上限；
- 将选择留给 `optimize` 分析器；
- 明确选择 `wait`，但不能依赖未声明的隐式等待。

事件间条件穿越只在条件和 `flow` 被静态证明为仿射时求精确有理根；其他连续条件变化必须拒绝，
不能延迟到下一个事件冒充穿越时刻。
...
Scenario 的常量位置（包括实例 input、variant、时点、周期和 fuel）可以引用同一工作区中
完全限定的静态 scalar field/output。Workspace 先按这些静态成员的源码默认 input 求得精确有理数或
布尔值，再把结果绑定为不可变 Process IR 字面量，同时保留原引用用于来源检查；无默认参数、非有理
结果和未声明 Package 依赖都会在加载时拒绝。它不是运行中可变的第二套参数通道。
...
runtime。容差是终止条件而非隐藏的固定步长，一般模型没有可验证的全局误差界，因此不能升级为
`global_with_error_bound`。静态退化为单点区间且完整枚举时才可返回 `exact_global`。

`exact_grid` 使用作者声明的正 `time_grid`，以场景时间零点为锚，枚举每个 `decide continuously`
区间内互不重复的网格时点、0..COUNT 次使用和全部动作组合。候选以精确有理时间进入同一 runtime；
完整穷举返回 `exact_global` 并在证明和运行记录中保存网格。若 `maximum_evaluations` 无法覆盖全部
候选，分析以 fuel 错误失败，不能降级成没有声明误差界的部分最优。求解器会记忆已经证明不可执行的
精确释放前缀，并跳过所有以该前缀开头的更长方案；这只消除必然重复的失败执行，不合并终局状态、
不删除可行并列最优，也不改变完整候选数和预算判定。证明记录分别给出候选、实际执行与剪枝方案数。

含有限随机 branch 与 `decide continuously` 时，`optimize` 搜索预先承诺的开放环方案。每个候选方案沿
完整有限随机树逐路径运行和计算 Measure，再以精确概率合成数值 Measure 期望；Objective 的数值项和
普通 `require` 在这些期望上求值，`require all_paths` 则要求条件在每条非零概率路径的 Measure 上成立。
`require probability at_least|at_most THRESHOLD: CONDITION` 精确合计满足条件的路径概率，可在期望约束
与绝对保证之间声明存活率下限或死亡率上限。最优结果保留候选释放表、完整加权结果集与原始路径计数、期望 Measure、
约束阈值和实际条件概率，不选择某一条路径冒充总体轨迹。

含有限随机 branch 与固定、事件后或条件决策时，`optimize` 改用精确可观测状态策略。它在每个公开
observation、可用动作和玩家既往动作构成的信息状态上选择动作，用 Bellman 递归求数值 Measure 期望，
并支持 `require all_paths` 保证。全局期望约束和 chance constraint 会耦合原本独立的信息状态，当前在
这一自适应证明模式中明确拒绝。事件和计数求和型目标按“到未来为止的增量”正规化后缓存，因此不同
历史累计值不会阻止等价未来状态合并；求得策略随后沿完整有限随机树精确重放，校验 Bellman 值并生成
结果分布。无可行保证策略时返回零个 `optimal_strategies`，其 `exact_global` 证明表示已经穷尽模型范围。

随机树执行会在每个时间/phase 批次前保留精确运行时检查点，并从该点展开本批分支，不再为每个叶子
从场景零点重放共同前缀。继续状态签名与每种轨迹 Measure 的充分历史签名共同控制精确状态聚合；概率
与原始路径数相加。含 trajectory chart 时自动禁用聚合以保留每条轨迹。结构化结果分别报告结果状态数、
原始路径数和每个状态代表的 `source_path_count`，分支总量仍受 `maximum_branches` 约束。
