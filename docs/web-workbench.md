...
- Documents covers CLI list/show/new/check and adds directory-grouped local sources, Package/version groups, per-document file actions, multi-document drafts, atomic Save All, explicit draft discard, external-change detection, integrated diagnostics, formula explanation, completion, result evaluation, named static-chart preview/export, and creation-time templates. An empty workspace replaces the three-pane editor with direct document-creation, syntax-reference, and external-authoring entry points.
- Relationship Graph derives global document and member projections from validated expression references and Process composition, then provides source navigation. Members include inputs, fields, functions, tables, finite distributions, objects, outputs, processes, scenarios, and analyses. Document projections use a deterministic circular layout; member projections retain a force layout. Selecting a node updates its semantic role, formula, source position, and direct upstream/downstream explanation without rebuilding or repositioning the graph. The document inspector shows a local zero-, one-, or two-hop projection from the same data, can limit traversal to dependencies or users, marks members of the current document, and reports both connection counts for the selected node.
- Syntax Reference is the single read-only reference center opened from the one navigation destination or command palette action. Contextual editor, diagnostic, and external-authoring links deep-link into that same center rather than creating separate help surfaces. Search covers Chinese labels, canonical syntax terms, official signatures, field names and constraints, external-Agent authoring boundaries, rule summaries, and example source. Each language topic first lists its public syntax items with context, required/optional status, allowed values, defaults, and a short field description, then retains overview rules and one complete copyable `.kirin` example. Copying never inserts into or otherwise mutates the active editor. The catalog is a versioned documentation projection whose structure and expected public-item set are regression-tested, while every bundled example is parsed and validated; the parser and validator remain the executable conformance boundary.
...
Large source-declared Process sweeps are explicitly started rather than automatically recalculated
on every edit. The workbench projects `family`, `enabled`, `vary` and `maximum_cases` into controls
that apply ordinary source drafts. The author sees the validated candidate count, starts comparison,
and can cancel through the existing job status; sweep progress reports actual completed/total cases.
Saved sources use "比较并保存运行记录"; dirty drafts use non-durable comparison. Ranking remains source-owned.
The result table paginates candidates, retains failed cases, shows rounded values alongside exact
scientific differences, and states the declared-grid scope. Selecting a row replays that case and
its source-declared trajectory charts through the same operation; full trajectories are not
precomputed for every candidate. Stale draft/selection responses cannot replace newer previews.

Static result Preview and Formula derive automatically from the current valid draft and never require Calculate, Generate Chart, or Explain Formula actions. The result projection may expose a compact `临时试算` area containing only inputs on the selected result's dependency path. Each row shows the source-authored default beside an optional temporary value. Temporary values live only in component memory, are passed to the existing comparison operation as an ephemeral overlay, do not enter draft recovery, and never modify `.kirin` or become a second authority. `重置` clears them. `生成 preset 草稿` explicitly appends an ordinary unsaved preset block to the one source editor; `保存运行记录` is available only when all drafts are clean and therefore records a durable source snapshot plus the requested overrides. Result and Chart selectors still choose projections rather than mathematical operations, and timeout or arbitrary advanced-operation forms remain absent. Explicit export controls remain separate because paths, overwrite, and workspace-boundary expansion require an intentional author action.
...
| Completion | `Ctrl+Space` | Searches legal candidates for the cursor context across built-ins, snippets, disk sources, and in-memory drafts |
...
The fold gutter collapses indented declaration blocks and prose blocks. One server-provided authoring contract supplies the editor vocabulary, operators, brackets, two-space indentation, line-comment form, Process built-ins, runtime Measure symbols, and reference identities. Completion sends the cursor location rather than only a word prefix, excludes comments, labels and prose, preserves static versus Process overloads on continuation lines, and includes nested Process/Scenario/Analysis symbols plus direction-checked instance paths and in-scope Process locals. Hover shows a symbol's canonical identity, kind, signature, and unit or domain detail; completion details, symbol hover, and diagnostics open the exact official reference item when known. Multiline and nested calls retain the active function signature and parameter number in the status bar. Focused and unfocused selections remain visibly distinct, and the insertion cursor and active line use the same ember interaction accent. The command palette also exposes Editor Only, Split, and Preview Only layout modes. Focus mode is remembered locally and changes only layout visibility, never source or projection semantics.

Safe rename updates the definition, local short references, and qualified cross-document references together while retaining Chinese aliases. It first produces ordinary in-memory `.kirin` overlays and validates the resulting workspace; Package definitions and Package source remain read-only. Formatting is deliberately source-preserving: it normalizes indentation tabs, trailing whitespace outside prose fences, excess blank lines, and the final newline without re-rendering or deleting comments. Known full-width punctuation diagnostics expose an explicit quick fix that edits only syntax outside quoted labels and comments. Diagnostics underline the nearest token instead of the rest of the line and are never rewritten automatically. If an edit arrives during validation, one pending validation is retained and runs when the current operation becomes idle, so the latest stopped draft is not left with a stale authoring index.
