...
- Added multiple derived charts per Process Analysis: same-unit trajectory overlays with public-event/decision markers, two-decision search surfaces, author-directed Pareto projections, and variant comparisons. Chart data is included in analysis results, while `kt analyze --export-charts` performs explicit workspace-confined SVG/CSV export.
