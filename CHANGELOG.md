...
- Added lightweight local-source revision polling so Agent- or tool-written `.kirin` documents appear automatically, clean buffers reload and revalidate, and dirty drafts open the existing conflict comparison without exposing Agent or CLI activity. The in-app authoring reference, command palette, empty-workspace guidance, and conflict copy now explain this file-mediated workflow and its authority limits.
