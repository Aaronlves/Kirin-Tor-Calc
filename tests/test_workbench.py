...
def test_empty_workbench_exposes_only_current_creation_templates(tmp_path: Path) -> None:
    root = initialize(tmp_path / "empty-workspace")
    workbench = Workbench(root)
    bootstrap = workbench.bootstrap()

    assert bootstrap["documents"] == []
    assert "tutorials" not in bootstrap
    assert all(item["origin"] != "tutorial" for item in bootstrap["templates"])
    assert {item["value"] for item in bootstrap["templates"]} == {
        "builtin:blank",
        "builtin:data",
        "builtin:model",
        "builtin:semantics",
